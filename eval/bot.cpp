...
GameState predict_next_state( const GameState &state, const GameState &prev_state, const vector<Command> & commands )
{
	...
	auto my_vel = get_vel( prev_state.my_ships, state.my_ships );
	auto enemy_vel = get_vel( prev_state.enemy_ships, state.enemy_ships );

	int n = (int)state.my_ships.size();
	for (int i=0; i<n; i++)
	{
		auto s = state.my_ships[i];
		auto & c = commands[i];
		auto s_v = predict_ship_state( s, my_vel[i], pp, c );
		if (s_v.s.cores > 0)
	...
	for (int i=0; i<m; i++)
	{
		auto s = state.enemy_ships[i];
		auto s_v = predict_ship_state( s, enemy_vel[i], pp, Command() );
		if (s_v.s.cores > 0)
...
vector<Command> bot_func_icarus(const GameState &old_state, const GameState &state)
{
	vector<Command> cmd;
	Point pp = { state.planet_size.x/2, state.planet_size.y/2 };
	auto my_vel = get_vel( old_state.my_ships, state.my_ships );
	//auto enemy_vel = get_vel( old_state.enemy_ships, state.enemy_ships );
...
{
	vector<Command> cmd;
	auto mv = bot_func_icarus(old_state, state);
	auto sh = bot_func_sniper(old_state, state);
...
{
	vector<Command> cmd;
	auto mv = bot_func_kamikaze(old_state, state);
	auto sh = bot_func_sniper(old_state, state);
	for (int i=0; i<(int)state.my_ships.size(); i++)
	{
		Command c;
...
vector<Command> bot_func_combine3(const GameState &old_state, const GameState &state)
{
	...
	{
		vector<Command> cmd;
		auto ka = bot_func_kamikaze2(old_state, state);
		auto ss = bot_func_combine(old_state, state);
		for (int i=0; i<(int)state.my_ships.size(); i++)
		{
			Command c;
			if (ka[i].explode)
...
int main()
{
	vector<Ship> my = { { { 100, 2 }, 0, 0, 64, 48, 24, 1, true } };
	vector<Ship> enemy = { { { -100, 2 }, 1, 0, 10, 10, 10, 1, false } };
	/*GameState gs = { Role::kAttacker, my, enemy, 255, { 10, 10 } };
	GameState prev_gs = { Role::kAttacker, my, enemy, 255, { 10, 10 } };

	for (int a=0; a<20; a++)
	{
		print_state( gs );
		vector<Command> cmd = { { false, true, {0,0}, {2,0} } };
		GameState new_gs = predict_next_state( gs, prev_gs, cmd );
		prev_gs = gs;
		gs = new_gs;
	}*/
