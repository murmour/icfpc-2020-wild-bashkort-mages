// Um, monads are really great. They are so elegant!
// Only morons don't understand monads.
// This page is a formal proof of it.
...
let start_t = chrono::high_resolution_clock::now();

void log_submission(const string s)
{
#ifdef SUBMISSION
	let end_t = chrono::high_resolution_clock::now();
...
Node* eval(Node *v)
{
	...
	case kCons2:
	{
		let x0 = f->arg1, x1 = f->arg2, x2 = v->arg2;
...
pair<bool, string> Node::getliststr()
{
	...
	if (ty == kCons2)
	{
		let s1 = eval(arg1)->getstr();
		let [is_list, s2] = eval(arg2)->getliststr();
		if (is_list)
...
Node* modem(Node *node)
{
	...
	if (node->ty == kCons2)
	{
		let a1 = modem(node->arg1);
		let a2 = modem(node->arg2);
...
{
	let res = eval(Ap(Ap(protocol, state), vector));
...
Node* demodulate_rec(const char *&s)
{
	...
	s += 2;
	let a = demodulate_rec(s);
	let b = demodulate_rec(s);
	return MakeNode(kCons2, a, b);
}
...
Node* send_inner(Node *data)
{
	...
	let res = demodulate(serverResponse->body.c_str());
...
Node* send_inner(Node *data)
{
	...
	log_submission("Sending:" + data->getstr());
#ifndef HTTPLIB
	let f = fopen("send.txt", "wt");
	fprintf(f, "%s", mdata.c_str());
	fclose(f);
#endif
#ifndef SUBMISSION
	let ff = fopen("sendlog.txt", "at");
	fprintf(ff, "%s\n", data->getstr().c_str());
	fclose(ff);
#endif
#ifdef _WIN32
...
#endif
	let resp = fopen("alien.txt", "rt");
	assert(resp);
	if (!fgets(alien_buffer, alien_buffer_size, resp))
		assert(false);
	fclose(resp);
	let res = demodulate(alien_buffer);
	log_submission("Got:" + res->getstr());
	return res;
}
#endif
...
{
	// stateless draw
	let protocol = parse("ap ap c ap ap b b ap ap b ap b ap cons 0 ap ap c ap ap b b cons ap ap c cons nil ap ap c ap ap b cons ap ap c cons nil nil");

	// stateful draw
	//let protocol = parse("ap ap b ap b ap ap s ap ap b ap b ap cons 0 ap ap c ap ap b b cons ap ap c cons nil ap ap c cons nil ap c cons");

	let state = parse("nil");
	let vector = parse("ap ap vec 0 0");
	let res = interact(protocol, state, vector);
	cout << "got " << res->getstr() << endl;

	state = n_th(res, 2);
	let vector2 = parse("ap ap vec 7 7");
	let res2 = interact(protocol, state, vector2);
...
void draw_image(FILE *f, Node *node)
{
	...
	for (let p : extract_list(node)) pts.push_back(extract_point(p));
	for (let [x, y] : pts)
	{
		minx = min(minx, x);
		miny = min(miny, y);
...
private:
	...
	void dfs(P p, set<P> &seen, vector<P> &pts) const
	{
		...
		{
			let t = make_pair(p.first + dx[dir], p.second + dy[dir]);
...
Ship get_ship_params(const Image &img, Point c)
{
	...
	int best = 1000;
	Point newc = c;
	for (let &sub : img.get_subimages())
	{
		let sc = sub.get_center();
...
struct Computation
{
	...
	void update(Point p)
	{
		...
		assert(state);
		let v = parse(format("ap ap vec %d %d", p.x, p.y).c_str());
		let res = interact(protocol, state, v);
		let imgs = modem(n_th(res, 1)->arg1);
		images.clear();
		int n_image = 0;
		for (let img : extract_list(imgs))
		{
#ifndef SUBMISSION
			let ifname = format("image_%d.log", n_image);
			let fimg = fopen(ifname.c_str(), "wt");
			draw_image(fimg, img);
			fclose(fimg);
#endif
			n_image++;
			vector<pair<int, int>> pts;
			for (let p : extract_list(img))
	...
	{
		vector<Image> res;
		for (let &v : images) res.push_back(v);
		return res;
	}
};
...
GameState get_full_game_state(Computation *comp)
{
	...
	for (let p : get_ships(images[0]))
		if (abs(p.x) > 10 || abs(p.y) > 10)
			defenders.push_back(p);
	let attackers = get_ships(images[2]);
	let &planet = images[4];
	...
	for (let p : defenders)
	{
		...
		if (comp->images.size() != 7)
			continue;
		let img = Image(comp->images[0]);
		img.ToString();
		let params = get_ship_params(img, p);
		if (params.id != -1)
...
void PlayGame(const string &url, const string &key)
{
	...
	let state = format("[5 [4 %s nil nil nil nil (36 0) 666] 9 %s]", key.c_str(), cheats);
	Computation comp(state);
	comp.update({ 27, 0 });
	log_submission("joined game");

	let ns = comp.get_state();
	...
	while (true)
	{
		// let new_st = MakeAMove(&comp, bot_func_combine, s);
		// let new_st = MakeAMove(&comp, bot_func_kamikaze, s);
		GameState new_st;
...
int main(int argc, char** argv)
{
	...
	if (argc > 2)
	{
		let url = string(argv[1]);
		arg_url = url;
		let key = string(argv[2]);
	...
	bool py = argc > 1 && string(argv[1]) == "py";

#ifndef SUBMISSION
	sprintf(fname, "interaction_%d.log", (int)time(nullptr));
	let log = fopen(fname, "wt");
#endif
	if (py)
	{
		while (true)
		{
			...
			let state = parse_list(state_buf);
			if (!state) exit(3);
			char buf[100];
			sprintf(buf, "ap ap vec %d %d", x, y);
			let vector = parse(buf);
			let res = interact(protocol, state, vector);
#ifndef SUBMISSION
			let images = modem(n_th(res, 1)->arg1);
			int n_image = 0;
			for (let img : extract_list(images))
			{
				draw_image(log, img);
				char ifname[1000];
				sprintf(ifname, "image_%d.log", n_image);
				let fimg = fopen(ifname, "wt");
				draw_image(fimg, img);
				fclose(fimg);
				n_image++;
			}
	...
	while (true)
	{
		let vector = parse("ap ap vec 1 1");
		if (interactive)
		{
			char *c = fgets(state_buf, buf_len, stdin);
            (void)c;
